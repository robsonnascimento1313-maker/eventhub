    ...
    <Layout>
      <div className="p-8">
        <div className="flex items-center justify-between mb-8">
          <div>
            <h1 className="text-2xl font-bold text-gray-100">Eventos</h1>
            <p className="text-gray-400 mt-1">Gerencie seus eventos corporativos</p>
          </div>
          <button
        ...
        {eventos.length > 0 ? (
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-5">
            {eventos.map((ev) => (
              <div key={ev.id} className="bg-surface rounded-xl border border-edge-soft shadow-sm p-5 hover:shadow-md transition">
                <div className="flex items-center justify-between">
                  <span className="w-10 h-10 rounded-lg bg-sky-400/10 text-sky-300 flex items-center justify-center">
                ...
                <h3 className="font-bold text-gray-100 mt-3">{ev.name}</h3>
          ...
          <div className="bg-surface rounded-xl shadow-sm border border-edge-soft p-12 text-center">
            <span className="inline-flex text-gray-600"><Icon name="calendar" size={44} strokeWidth={1.5} /></span>
            <h3 className="text-lg font-semibold text-gray-200 mt-4">Nenhum evento cadastrado</h3>
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50 p-4" onClick={() => setOpen(false)}>
          <div className="bg-surface rounded-2xl shadow-xl w-full max-w-md" onClick={(e) => e.stopPropagation()}>
            <div className="flex items-center justify-between p-6 border-b border-edge-soft">
              <h3 className="font-bold text-gray-100">Novo evento</h3>
              <button onClick={() => setOpen(false)} className="text-gray-400 hover:text-gray-300"><Icon name="x" size={20} /></button>
            </div>
            <form onSubmit={handleCreate} className="p-6 space-y-4">
              ...
              <div>
                <label className="block text-sm font-medium text-gray-200 mb-1">Local</label>
                <input
                  value={form.location}
